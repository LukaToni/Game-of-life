...
//neig_Xe cell (allive + neighbors)
int all_X[8000];
int all_Y[8000];
int i_of_cell[8000];
int alive_cell, i, j;

int neig_X[8]={-1,-1 , -1,  0, 1,  1, 1, 0}; //neighbors
...
void neig_XeCel()
{
	...
	for(i=0; i<alive_cell_num; i++)
	{
		...
		{ //8 neighbors
			...
			{ // not allived neighbors add him in table
	...
	all_X[alive_cell] = x_live[i]; //allived in table neig_Xeh
